...
const TYPE_OPTIONS = ['all', 'dine-in', 'dine_in', 'takeaway', 'preorder']
...
export default function OrdersAdmin() {
  ...
  const exportCSV = () => {
    const rows = [['Order #', 'Customer', 'Phone', 'Type', 'Status', 'Amount', 'Date']]
    filtered.forEach(o => rows.push([o.order_number, o.customer_name, o.whatsapp_number || o.customer_phone, o.order_type, o.order_status, String(o.subtotal), o.created_at]))
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {['Order #', 'Customer', 'Type', 'Status', 'Amount', 'Time', ''].map(h => (
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map(o => (
                    <tr key={o.id} onClick={() => setSelected(selected?.id === o.id ? null : o)}
                      ...
                      <td className="px-4 py-3 text-charcoal/60 capitalize">{o.order_type?.replace('_', '-')}</td>
                      ...
                      <td className="px-4 py-3 text-charcoal/50 text-xs">{new Date(o.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</td>
                      <td className="px-4 py-3">
        ...
        {selected && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between"><span className="text-charcoal/50">Order #</span><span className="font-semibold text-saffron">{selected.order_number}</span></div>
            ...
            <div className="flex justify-between font-bold text-base border-t border-gray-100 pt-2">
              <span>Total</span><span className="text-saffron">₹{selected.subtotal}</span>
            </div>
