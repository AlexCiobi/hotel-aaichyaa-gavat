...
const WAITER_PIN = import.meta.env.VITE_WAITER_PIN || '1234'
...
export default function WaiterLogin() {
  ...
  function handleLogin(e: React.FormEvent) {
    ...
    localStorage.setItem('th_waiter', JSON.stringify({ name: name.trim(), loggedInAt: Date.now() }))
    navigate('/waiter')
  }
    ...
    <div className="min-h-screen bg-[#1A1A1A] flex items-center justify-center p-4">
      <form onSubmit={handleLogin} className="w-full max-w-sm bg-[#222] rounded-2xl p-8 shadow-2xl">
        <div className="flex items-center justify-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-white font-bold text-xl">Waiter Portal</h1>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-white/60 text-xs font-semibold uppercase tracking-wider block mb-2">Your Name</label>
            <input
              value={name} onChange={e => { setName(e.target.value); setError('') }}
              placeholder="e.g. Raj"
              className="w-full bg-white/10 text-white rounded-xl px-4 py-3 text-sm border border-white/10 focus:border-[#C0272D] focus:outline-none placeholder:text-white/30"
            />
          </div>
          <div>
            <label className="text-white/60 text-xs font-semibold uppercase tracking-wider block mb-2">PIN</label>
            <input
